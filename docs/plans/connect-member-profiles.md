...
done_criteria:
  ...
  - id: dc5
    ...
    verify: test -s /tmp/rmb-home-390.png && test -s /tmp/rmb-home-1440.png && test -s /tmp/rmb-connect-390.png && test -s /tmp/rmb-connect-1440.png && test -s /tmp/rmb-member-390.png && test -s /tmp/rmb-member-1440.png && test -s /tmp/rmb-progress-390.png && test -s /tmp/rmb-progress-1440.png && rg -q "Adults|Connect|Matthew" /tmp/rmb-after-home.snapshot.txt /tmp/rmb-after-connect.snapshot.txt /tmp/rmb-after-member.snapshot.txt /tmp/rmb-after-progress.snapshot.txt
milestones:
  - id: m1
    criteria: [dc1]
  ...
  - id: m3
    ...
    lands_on: feat/connect-profile-calendar-v3
named_actions:
  - action: DEV_MOCK_PERSON_ID=152 DEV_MOCK_TODAY=2026-10-08 pnpm dev --port 3011; agent-browser --session readmybible-connect-after open http://localhost:3011; agent-browser --session readmybible-connect-after wait 2000; agent-browser --session readmybible-connect-after viewport 390 844; agent-browser --session readmybible-connect-after screenshot --full /tmp/rmb-home-390.png; agent-browser --session readmybible-connect-after snapshot -i > /tmp/rmb-after-home.snapshot.txt; agent-browser --session readmybible-connect-after viewport 1440 900; agent-browser --session readmybible-connect-after screenshot --full /tmp/rmb-home-1440.png; use the returned Connect control from `snapshot -i`, capture `/tmp/rmb-connect-390.png` and `/tmp/rmb-connect-1440.png`, activate one roster avatar and capture `/tmp/rmb-member-390.png` and `/tmp/rmb-member-1440.png`, then activate Progress and capture `/tmp/rmb-progress-390.png` and `/tmp/rmb-progress-1440.png`, saving a `snapshot -i` after each screen to the matching `/tmp/rmb-after-*.snapshot.txt` file
    ...
    revert: kill the named local dev-server process and remove `/tmp/rmb-*.png` and `/tmp/rmb-after-*.snapshot.txt`
    read_back: test -s /tmp/rmb-home-390.png && test -s /tmp/rmb-home-1440.png && test -s /tmp/rmb-connect-390.png && test -s /tmp/rmb-connect-1440.png && test -s /tmp/rmb-member-390.png && test -s /tmp/rmb-member-1440.png && test -s /tmp/rmb-progress-390.png && test -s /tmp/rmb-progress-1440.png && rg -q "Adults|Connect|Matthew" /tmp/rmb-after-home.snapshot.txt /tmp/rmb-after-connect.snapshot.txt /tmp/rmb-after-member.snapshot.txt /tmp/rmb-after-progress.snapshot.txt
